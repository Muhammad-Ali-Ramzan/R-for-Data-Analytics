...
model1$Group <- as.factor(model1$Group)
model1$Score <- as.factor(model1$Score)
#2
df2 <- data.frame(
  Group = rep(c("A", "B", "C"), each = 10),
...
# head(chickwts)
# summary(chickwts)
# ch <- aov(weight ~ feed, data = chickwts)
# ch
# chtk <- TukeyHSD(ch)
# chtk
# plot(chtk)
...
#2
model2 <- aov(Score ~ Group, data = df2)
model2
tk2 <- TukeyHSD(model2)
tk2
plot(tk2)
...
#3
model3 <- aov(Score ~ Group, data = df3)
model3
tk3 <- TukeyHSD(model3)
plot(tk3)
plot(tk3, las = 2)
...
#4
model5 <- aov(Score ~ Group, data = df4)
model5
plot(model5)
